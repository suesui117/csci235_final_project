...
/**
    ...
    @param            :   A POINTER to an integer counter that gets incremented every time a SWAP between two Nodes occurs
                        ...
                        IT IS IMPORTANT TO PASS OUR GRADESCOPE TESTS THAT EACH
...
LinkedList<Post*> Network<ItemType>::bSortByUsername(Comparator cmp, int *counter){

    LinkedList<Post*> new_feed(feed_); // Copying the existing feed_ to another feed - that is new_feed - of the type Post Pointers.

    new_feed.bubbleSort(cmp, counter); // perform bubbleSort on new_feed (copied feed_).
