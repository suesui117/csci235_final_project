...
Post::Post(std::string title, std::string body, std::string username) {
    ...
    username_ = username;
    // likes_ = 0;
    // By default, set the enum variable - priority_id_ to MEDIUM. (Here priority_id_ is the variable of the enum named priority).
    // priority_id_ = MEDIUM;
}
...
bool Post::operator<(const Post& a_post) const{
    return this->getUsername() < a_post.getUsername();
}
...
*/
void Post::setPriority(priority a_value){
    priority_id_ = a_value;
}
...
*/
Post::priority Post::getPriority(){
    return priority_id_;
}
